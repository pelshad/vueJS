      ...
      <div class="mb-3 row">
        ...
        <div class="col-md-9">
          ...
          <input type="file" 
                class="form-control" 
                accept="image/png,image/jpeg" 
                @change="uploadFile($event.target.files, 2)"> <!--multiple 속성을 주면 여러개 올릴 수 있음-->
          <div class="alert alert-secondary" role="alert">
            <ul>
              <li>최대 5개 가능</li>
    ...
    async deleteImage(id, product_id, type, path) {
      this.$swal.fire({
        ...
        if(result.isConfirmed) {
          const imagenm = await this.$delete(`/api/productImageDelete/${id}/${product_id}/${type}/${path}`); //pathvariable 방식
          console.log(imagenm)
          this.getProductImage();
